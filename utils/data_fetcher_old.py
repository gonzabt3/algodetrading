...
class DataFetcher:
    ...
    def fetch_historical_data(self, symbol: str, days: int = 365, db = None) -> Optional[pd.DataFrame]:
        """
        Fetch historical data - tries DB first, then API, and saves to DB
        ...
        # Si tenemos DB, intentar primero desde ahí
        if db is not None:
            try:
                from api import crud
                
                # Obtener datos de la DB
                db_data = crud.get_market_data(
                    db=db,
                    symbol=symbol,
                ...
                if db_data and len(db_data) > 0:
                    ...
                    # Si tenemos datos recientes (últimos 2 días), usarlos
                    latest = df.index.max()
                    if (end_date - latest).days <= 2:
                        self.logger.info(f"Using database data (up to date until {latest})")
                        return df
                    else:
                        self.logger.info(f"Database data is outdated (latest: {latest}), fetching from API")
            except Exception as e:
                self.logger.error(f"Error loading from database: {e}")
        
        # Obtener desde API
        self.logger.info(f"Fetching live data from exchange for {symbol}, {days} days")
        df = self.fetch_ohlcv(symbol, '1d', start_date, end_date)
        
        if df is not None and not df.empty:
            self.logger.info(f"Successfully fetched {len(df)} records from exchange")
            ...
            if db is not None:
                try:
                    ...
                    count = crud.bulk_save_market_data(db, data_list)
                    self.logger.info(f"Saved {count} records to database")
                except Exception as e:
