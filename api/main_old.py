...
async def get_pair_data(
    ...
    """
    Obtener datos de dos símbolos para visualización de pair trading.
    """
    try:
        start = datetime.fromisoformat(start_date)
        end = datetime.fromisoformat(end_date)
        
        # Obtener datos de ambos símbolos
        data_a = crud.get_market_data(db, symbol_a, start, end, timeframe='1d')
        data_b = crud.get_market_data(db, symbol_b, start, end, timeframe='1d')
        
        if not data_a or not data_b:
            raise HTTPException(status_code=404, detail="No se encontraron datos para uno o ambos símbolos")
        ...
        if len(merged) == 0:
            raise HTTPException(status_code=404, detail="No hay timestamps coincidentes entre los símbolos")
        
        # Normalizar precios para comparación visual (base 100)
...
async def run_backtest(request: BacktestRequest, db: Session = Depends(get_db)):
    """Run a new backtest (supports both single and multi-symbol strategies)"""
    ...
    try:
        ...
        # Check if multi-symbol strategy
        is_multi_symbol = strategy_info.get('type') == 'multi_symbol'
        
        if is_multi_symbol:
            # Handle pair trading strategy
            symbol_a = request.symbol_a or strategy_info.get('default_symbols', [])[0]
            symbol_b = request.symbol_b or strategy_info.get('default_symbols', [])[1]
            
            if not symbol_a or not symbol_b:
                raise HTTPException(
                    status_code=400,
                    detail="Pair trading requires symbol_a and symbol_b"
                )
            ...
            print(f"Fetching data for pair: {symbol_a}, {symbol_b}...")
            start_date = datetime.now() - timedelta(days=request.days)
            
            data_a = data_fetcher.fetch_historical_data(
                symbol=symbol_a,
                start_date=start_date
            )
            data_b = data_fetcher.fetch_historical_data(
                symbol=symbol_b,
                start_date=start_date
            )
            ...
            if data_a is None or data_a.empty or data_b is None or data_b.empty:
                raise HTTPException(
                    status_code=400,
                    detail="Failed to fetch market data for one or both symbols"
                )
            
            # Align data (inner join on timestamps)
            common_index = data_a.index.intersection(data_b.index)
            data_dict = {
                symbol_a: data_a.loc[common_index].copy(),
                symbol_b: data_b.loc[common_index].copy()
            }
            
            # Create backtest record (use first symbol for DB)
            backtest = crud.create_backtest(
                db=db,
                strategy_id=db_strategy.id,
                symbol=f"{symbol_a}_{symbol_b}",  # Combine symbols
                params=request.params if request.params else strategy_info['params'],
                initial_capital=request.initial_capital,
                commission=request.commission,
                start_date=common_index[0].to_pydatetime() if hasattr(common_index[0], 'to_pydatetime') else common_index[0],
                end_date=common_index[-1].to_pydatetime() if hasattr(common_index[-1], 'to_pydatetime') else common_index[-1]
            )
            ...
            # Handle single-symbol strategy (original code)
            await manager.broadcast({
                'type': 'backtest_started',
                'strategy': request.strategy_id,
